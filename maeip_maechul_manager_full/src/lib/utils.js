...
export function makeSummary(transactions, documents, month) {
  ...
  const income = tx.filter(t => t.type === 'income' || t.category === '매출').reduce((sum, t) => sum + Number(t.amount || 0), 0)
  const expense = tx.filter(t => t.type !== 'income' && t.category !== '매출').reduce((sum, t) => sum + Number(t.amount || 0), 0)
